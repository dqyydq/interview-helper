    ...
    <section className="diagnostics-page" aria-labelledby="diagnostics-title">
      ...
      {snapshot && (
        <div className="diagnostics-grid">
          <article>
            <header><Database size={18} /><span>数据库</span><StatusMark ok={snapshot.database.status === "connected"} /></header>
            <strong>{snapshot.database.status === "connected" ? "PostgreSQL 已连接" : "PostgreSQL 不可用"}</strong>
            <p>诊断不会输出数据库地址或凭据。</p>
          </article>
          <article>
            <header><ServerCog size={18} /><span>后台任务</span><StatusMark ok={snapshot.worker.state === "healthy"} /></header>
          ...
          <article>
            <header><Activity size={18} /><span>模型路由</span><StatusMark ok={snapshot.models.required_ready} /></header>
            ...
            <small>语音转写 {snapshot.models.transcriber_configured ? "已配置" : "未配置（文字输入可用）"}</small>
          </article>
          <article>
            <header><FolderLock size={18} /><span>本地文件</span><StatusMark ok={snapshot.files.exists && snapshot.files.writable} /></header>
          ...
          <article className="diagnostics-privacy">
            <header><ShieldCheck size={18} /><span>隐私检查</span><StatusMark ok={snapshot.privacy.redaction_applied} /></header>
